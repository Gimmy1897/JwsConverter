    ...
    if len(sys.argv) > 1:
        ...
        if(path.exists(folder_path)):
            ...
            for root, dirnames, filenames in os.walk(folder_path,topdown=True):
                dirnames.clear()
                if len(filenames) > 0:
                    if(any(File.endswith(".jws") for File in filenames)):
                        for filename in filenames:
                            if filename.endswith('.jws'):
                                matches.append(os.path.join(root, filename))

                        for jws_file in matches:
                            print("[JwsConverter]: Conversion of ----> ",jws_file.rsplit('\\',1)[1])
                            main(jws_file)
                            print("[JwsConverter]: Done :))\n______\n")
                    else: print ("[JwsConverter ERROR]: no .jws file found in the selected folder :/ (",root,") change path!")

                else: print ("[JwsConverter ERROR]:  This folder is empty :/ (",root,") change path!")
    else:
        print ('USAGE:py jwsconverter.py input_folder_path')
